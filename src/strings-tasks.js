...
function sumOfCodes(str) {
  return str ? str.split('').reduce((acc, el) => acc + el.charCodeAt(), 0) : 0;
}
...
function countVowels(str) {
  return str
    .toLowerCase()
    .split('')
    .reduce((acc, el) => ('aeiouy'.split('').includes(el) ? acc + 1 : acc), 0);
}
...
function isPalindrome(str) {
  const modyfiedStr = str.toLowerCase().replace(/[ .,?!]/g, '');
  return modyfiedStr === modyfiedStr.toLowerCase().split('').reverse().join('');
}
